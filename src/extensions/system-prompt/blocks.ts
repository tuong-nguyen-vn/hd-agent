...
export const SUBAGENT_DELEGATION_BLOCK = [
  ...
  "Once the user approves, gather context first — yourself or via Search subagents — then send a self-contained brief: Objective, Scope/files, Known findings with quoted evidence (cited as path:line), Constraints, and Questions. Pass the supporting source files via the subagent tool's context_paths; files the brief cites as path:line are inlined automatically.",
  "Mark each finding VERIFIED when you read the code yourself and HYPOTHESIS when it came from a Search subagent and you have not checked it — an unmarked guess presented as fact sends Oracle chasing a false lead.",
  "Keep the brief to what needs judgement. Items you have already proven and can simply fix — stale docs, a missing export, an untested branch — do not belong in an Oracle consultation.",
