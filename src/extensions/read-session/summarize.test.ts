...
test("summarizeSession uses gemini-3.8-flash when available", async () => {
  const primary = model("google", "gemini-3.8-flash");
  const calls: string[] = [];
...
test("summarizeSession falls back to the main model after primary failure", async () => {
  ...
  expect(calls).toEqual(["google/gemini-3.8-flash", "openai/main"]);
});
