...
- Clone tracks `main`; if behavior diverges from the installed version, check `node_modules/@earendil-works/pi-coding-agent/package.json` and `git -C vendor/pi checkout <tag>`. Current pinned version: **0.87.0**.
...
**Images from an extension**: pi renders images only from tool results and from custom messages — assistant messages have no image block or event (`docs/custom-provider.md` "Event Types"). `CustomMessage.content` accepts `ImageContent`; `pi.sendMessage({ content: [{ type: 'image', … }] }, { triggerTurn: false })` from a `turn_end` handler lands right after the assistant message with no extra LLM call, reaches the model as a user-role image, and is drawn by `pi.registerMessageRenderer` (use `StableImage`; build the base64 with `ImagePreview.build` — a full-size generated image is megabytes of escape sequence per frame). `ProviderConfig.streamSimple` + `ProviderRequestOptions.fetch` let a provider tee its own SSE stream. pi applies a model's `inputLimits.images.resize` to tool-result images but not to custom messages, so resize with `resizeImage` (exported by pi-coding-agent) before `sendMessage`; the proxy providers share one profile in `src/extensions/provider/image-limits.ts`. See `src/extensions/native-image/`.
...
- `ImagePreview` — `build(base64, mimeType, PimSettings.getImagePreview())`: downscales an image to the cell box the TUI draws it in (photon) and, by default, quantises it to a 256-colour palette PNG via `IndexedPng`; `undefined` when previews are off (`imagePreview.maxWidthCells: 0`) or the terminal has no image protocol. Always run generated/read images through it before handing them to a component, and pass the returned `widthCells` to the component.
- `IndexedPng` — `encode(rgba, width, height)`: 8-bit palette PNG encoder (histogram median cut + Floyd–Steinberg); a photo-like RGB PNG comes out ~3x smaller.
- `StableImage` — pi-tui `Image` that ignores the `invalidate()` cascade, so a theme change or chat rebuild does not make the terminal re-upload the whole payload.
