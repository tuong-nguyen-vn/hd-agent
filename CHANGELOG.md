...
- Add `deepseek-v4-1-flash` to hdwebsoft-proxy (300K context, 131K max output).
- Remove `gemini-3.1-pro` from hdwebsoft-proxy.
- Remove `glm-5-3` from hdwebsoft-proxy; `glm-5-3-flash` stays, with its context window raised from 200K to 300K.
...
- Native image generation in the main agent: when the active model is in `nativeImageGen.models` (default `gpt-5.6-luna`) on a bundled proxy, the proxy's `image_generation` tool rides along on the model's chat requests. Both proxies now register a `streamSimple` that tees the SSE stream (`NativeImageCapture`), keeping the `images` field pi-ai's completions driver would drop; the new `native-image` extension saves each image as `./image-<timestamp>.jpg`, posts it back as an image-bearing custom message rendered inline via pi-tui `Image` (PNG preview for kitty), fills an image-only reply with a placeholder line (in headless runs the saved path is appended to the reply instead of queuing a message behind it), and hides `painter` while a native model is active (`setActiveTools` on `session_start`/`model_select`). No pi internals are patched. `nativeImageGen.enabled` turns it off.
- painter now speaks one protocol: `/responses` with the `image_generation` tool (`gpt-5.6-luna`), dropping `/images/*` and Gemini `/interactions`. Calls in a session chain on `previous_response_id` for text context and prompt cache; `mode: "edit"` without `input` re-attaches the last painter image (the proxy's image tool only sees images in the current request), and `continue_session: false` starts fresh. Local inputs are inlined as data URIs, http(s) inputs pass through by URL, unset size/quality/format are omitted so the server chooses, and the output extension follows the format the server reports. The `painter.model` default is now `gpt-5.6-luna`.
